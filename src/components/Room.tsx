...
export default function Room({imgURL, name, rating, city, price, country}:RoomProps){
        ...
        <VStack alignItems={"flex-start"} spacing={-1}>
            <Box>
                <Box position={"relative"} overflow={"hidden"} rounded={"3xl"}>
                    ...
                    <Button variant={"unstyled"} position={"absolute"} top={1} right={0.7} color={"white"}>
                        <FaRegHeart size={"20px"} />
                    </Button>
                </Box>
            </Box>
            <Grid gap="2" templateColumns={"5fr 1fr"}>
                <Heading fontSize={"s"} noOfLines={1}>{name}</Heading>
                <HStack spacing={1}>
                    <FaStar/><Text fontSize={"sm"}>{rating}</Text>
                </HStack>   
            </Grid>
            
            <Text fontSize={"sm"} color={gray}>{city},{country}</Text>
            <Text fontSize={"sm"} color={gray}>23-28 Feb</Text>
